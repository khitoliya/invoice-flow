...
class EInvoiceMapper @Inject constructor() {
    ...
    fun mapInvoiceToEInvoiceDraft(
        ...
        return EInvoiceDraft(
            ...
            DocDtls = DocDtls(
                ...
                Dt = LocalDate.parse(invoice.invoiceDate, DateTimeFormatter.ofPattern("dd-MM-yyyy")).format(DATE_FORMAT)
            ),
            ...
            EwbDtls = if (invoice.generateEWayBill) {
                EwbDtls(
                    ...
                    Distance = invoice.transportDetails.distance,
                    TransDocNo = invoice.transportDetails.transporterDocNo,
                    TransDocDt = invoice.transportDetails.transporterDocDate?.let { 
                         // Ensure format? 
                         LocalDate.parse(it, DateTimeFormatter.ofPattern("dd-MM-yyyy")).format(DATE_FORMAT) 
                    },

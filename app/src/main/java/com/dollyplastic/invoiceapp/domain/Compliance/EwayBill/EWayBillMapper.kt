...
class EWayBillMapper @Inject constructor() {
    ...
    fun mapInvoiceToEWayBillDraft(
        ...
        val docDate = LocalDate
            .parse(invoice.invoiceDate, DateTimeFormatter.ofPattern("dd-MM-yyyy"))
            .format(NIC_DATE_FORMAT)

        val transportDocDate =
            if (!transport.transporterDocDate.isNullOrBlank()) {
                LocalDate.parse(transport.transporterDocDate, DateTimeFormatter.ofPattern("dd-MM-yyyy"))
                    .format(NIC_DATE_FORMAT)
            } else {
        ...
        return EWayBillDraft(
            ...
            transDistance = invoice.transportDetails.distance,
